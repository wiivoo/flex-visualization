...
import { parseEnabledCountry, type AppCountry } from '@/lib/country-config'
...
function parseCountry(params: URLSearchParams, enableGb: boolean): AppCountry {
  return parseEnabledCountry(params.get('country'), enableGb)
}
...
export function V2PageClient({
  ...
  return <Suspense><V2Inner enableGb={enableGb} enableIntraday={enableIntraday} /></Suspense>
}

function V2Inner({ enableGb, enableIntraday }: { enableGb: boolean; enableIntraday: boolean }) {
  ...
  const [country, setCountry] = useState<AppCountry>(() => parseCountry(searchParams, enableGb))
  const [gbAuction, setGbAuction] = useState<GbDayAheadAuction>(() => parseGbAuction(searchParams))
  const [resolution, setResolution] = useState<Resolution>(() => {
    const initialCountry = parseCountry(searchParams, enableGb)
    const initialAuction = parseGbAuction(searchParams)
    return parseResolution(searchParams, initialCountry, initialAuction)
  })

  useEffect(() => {
    if (!enableGb && country === 'GB') {
      setCountry('DE')
      setResolution('hour')
    }
  }, [country, enableGb])

  const prices = usePrices(country, gbAuction, enableIntraday)
