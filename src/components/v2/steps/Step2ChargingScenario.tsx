...
import { VEHICLE_PRESETS, type ChargingScenario } from '@/lib/v2-config'
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-sm">
        ...
        <span className="font-semibold text-[#313131]">{value}{unit}</span>
      </div>
      <input
        type="range"
        min={min}
...
export function Step2ChargingScenario({ prices, scenario, setScenario, optimization, onNext, onBack }: Props) {
  ...
  const chartData = useMemo(() => {
    ...
    // Build a map of which hours have baseline/optimized charging
    const baselineHours = new Set<string>()
    const optimizedHours = new Set<string>()
    ...
    if (optimization) {
      for (const block of optimization.baseline_schedule) {
        ...
        // Simple: mark start hour
        for (let h = startH; h !== endH; h = (h + 1) % 24) {
          baselineHours.add(String(h).padStart(2, '0'))
      ...
      for (const block of optimization.charging_schedule) {
        ...
        const endH = parseInt(block.end.split(':')[0])
        for (let h = startH; h !== endH; h = (h + 1) % 24) {
          optimizedHours.add(String(h).padStart(2, '0'))
    ...
    return allPrices.map(p => {
      ...
      const isInWindow = isHourInWindow(p.hour, scenario.plugInTime, scenario.departureTime)
      return {
        hour: `${hourKey}:00`,
  ...
  }, [prices.selectedDayPrices, optimization, scenario])

  const annualSavings = optimization ? Math.round(optimization.savings_eur * 200) : 0
  const sessionsPerYear = 200
    ...
    <div className="space-y-6">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-lg text-gray-500 max-w-2xl mx-auto">
          Adjust the scenario below and see how shifting charging to the cheapest hours creates real savings.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <Card className="col-span-2">
          ...
          <CardContent>
                ...
                <ComposedChart data={chartData} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(val, name) => {
                      ...
                      return [`${Number(val).toFixed(1)} EUR/MWh`, name === 'baseline' ? 'Baseline' : name === 'optimized' ? 'Optimized' : 'Price']
                    }}
                  />
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg text-sm text-gray-700">
              ...
              <p className="mt-1">
                <span className="inline-block w-3 h-3 bg-green-500 rounded mr-1 align-middle" /> <strong>Green bars</strong> show our optimized schedule — charging shifted to the cheapest available hours. Same energy, lower cost.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {VEHICLE_PRESETS.map(v => (
                <button
                  key={v.id}
                  onClick={() => setScenario({ ...scenario, vehicleId: v.id })}
                  className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-all ${
                    scenario.vehicleId === v.id
                      ? 'border-[#EA1C0A] bg-[#EA1C0A]/5 text-[#EA1C0A]'
                      : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
                  <div className="font-semibold">{v.label}</div>
                  <div className="text-xs text-gray-500">{v.battery_kwh} kWh · {v.charge_power_kw} kW · {v.examples}</div>
                </button>
              ))}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Charging Scenario</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <SliderInput
                label="Plug-in Time"
                value={scenario.plugInTime}
                min={14} max={23} step={1} unit=":00"
                onChange={(v) => setScenario({ ...scenario, plugInTime: v })}
              />
              <SliderInput
                label="Departure Time"
                value={scenario.departureTime}
                min={4} max={10} step={1} unit=":00"
                onChange={(v) => setScenario({ ...scenario, departureTime: v })}
              />
              <SliderInput
                label="Start Level"
                value={scenario.startLevel}
                min={10} max={80} step={5} unit="%"
                onChange={(v) => setScenario({ ...scenario, startLevel: v })}
              />
              <SliderInput
                label="Target Level"
                value={scenario.targetLevel}
                min={50} max={100} step={5} unit="%"
                onChange={(v) => setScenario({ ...scenario, targetLevel: v })}
              />
            </CardContent>
          </Card>
      ...
      <div className="flex justify-between items-center pt-4">
        <Button variant="outline" onClick={onBack}>&larr; Back: Price Curve</Button>
